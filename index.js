...
exports.update = function (model, query, data, done) {
  model.findOneAndUpdate(query, data, {new: true}, function (err, o) {
    if (err) {
      return done(err);
    }
    if (!o) {
...
exports.findOne = function (model, query, done) {
  model.findOne(query).exec(function (err, o) {
    if (err) {
      return done(err);
    }
    if (!o) {
...
exports.remove = function (model, query, done) {
  model.remove(query).exec(function (err, o) {
    if (err) {
      return done(err);
    }
    if (!o.n) {
